...
if [ -f ${PROJECT_DIR}/.pio/build/esp32dev/firmware.hex ]; then
  mv ${PROJECT_DIR}/.pio/build/esp32dev/firmware.hex ${BINDIR}/attenuator/Attenuator-ESP32.hex
fi
echo "Done."
echo ""
...
if [ -f ${PROJECT_DIR}/.pio/build/esp32dev/firmware.hex ]; then
  mv ${PROJECT_DIR}/.pio/build/esp32dev/firmware.hex ${BINDIR}/attenuator/Attenuator-ESP32-Standalone.hex
fi
...
if [ -f ${PROJECT_DIR}/.pio/build/esp32dev/firmware.hex ]; then
  mv ${PROJECT_DIR}/.pio/build/esp32dev/firmware.hex ${BINDIR}/attenuator/extras/Attenuator-ESP32-Reset.hex
fi
